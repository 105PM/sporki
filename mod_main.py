...
from .setup import *
from .sporki_handle import Sporki
...
class ModuleMain(PluginModuleBase):
    ...
    def process_menu(self, page_name, req):
        ...
        return render_template(
            f"{P.package_name}_{self.name}_{page_name}.html", arg=arg
        )

    def process_command(self, command, arg1, arg2, arg3, req):
        if command == "broad_list":
            ret = {"ret": "success", "ch_list": Sporki.ch_list()}
        elif command == "play_url":
            url = ToolUtil.make_apikey_url(
                f"/{P.package_name}/api/url.m3u8?ch_id={arg1}"
            )
    ...
    def process_api(self, sub, req):
        try:
            if sub == "m3u":
                ...
                mode, data = Sporki.get_m3u8(req.args.get("ch_id"))
                if mode == "text":
                    return data
                else:
                    return redirect(data)
            elif sub == "segment.ts":
                return Sporki.segment(req)
